...
class BotElINGen():
    ...
    def __init__(self, prnt=None, maxforces=None):
        ...
        self.types = {"angel":["Serꙮph".decode("utf-8"),"Cherub","Ofanite","Elohite","Malakite","Kyriotate","Mercurian","Bright Lilim"],
                      "demon":["Balserꙮph".decode()"utf-8"),"Djinn","Calabite","Habbalite","Lilim","Shedite","Impudite"]}
    ...
    def createCharacter(self, name=None, side=None, ret=False):
        ...
        for realm in abilities.keys():
            while sum(abilities[realm].values()) < ability_points[realm]:
                abl_tgt = random.choice(list(abilities[realm].keys()))
                if abilities[realm][abl_tgt] < 12:
                    abilities[realm][abl_tgt] += 1
